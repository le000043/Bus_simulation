 ...
 *
 * USER Guide:
 * The purpose of this program is to let users simulate a bus simulation. In the simulation, passengers will 
 * appear at stops based on a determined probability. Each bus has two routes which are outgoing and incoming.
 * Each stop will consist of a list of stops. Each bus will pick up and drop off passengers at each stop.
 * The user will later be guided how to customize routes, stops, probability of generating passengers at each stop,
 * the length of simulation, and the time between busses in each route. The user is expected to use CSE Linux lab machine to run the simulation.
 * There are two ways to visualize a simulation. They are running a simulation with a visualizer(vis_sim) and running a simulation without a visualizer(config_sim).
 * Vis_sim is for those who want to visualize the simulation. Config_sim is for those who wish to control their simulation via terminal and/or run the simulation on a non-default configuration file.
 * The sequence of steps below will guide users on three options: running a simulation, customizing default configuration file, or
 * creating and then running simulation on a new configuration file.
 *
 * I. Installation
 * 
 ...
 * 
 * b) Running simulation with configuration file
 * 1. Type "cd.." to go back to src directory.Then type "./build/bin/configuration_sim". Hit Enter to run the default configuration file (config.txt) inside the config folder which is in the project folder.
 * 2. A simulation with default configuration will be shown in the current terminal window. Default length of simulation is 25 time steps and time between busses is 5 time steps.
 * 3. To run the defaut configuration with different length of simulation or time between busses, refer to step 17 of "a) Overview of Configuration file".
 * 4. The following command is an example to run the simulation 10 time steps with 2 as the time between busses in East bound route and 3 as the time between busses in West bound route: "./build/bin/configuration_sim config.txt 10 2 3". Then hit Enter. Result of simulation is on the current terminal window.
 * 5. If user mistypes the configuration file, the program will let user know there are typos and will exit. Follow step 4 of "b) Running simulation with configuration file" to type the correct command with correct file name.
 * 6. If more arguments are given than it needs, the program will exit. Refer to step 4 of "b) Running simulation with configuration file" to re-run the program.
 * 7. If there are not enough given arguments, the program will run the simulation with default configuration file which is config.txt.
 * 8. If users make any changes in the configuration file that is used for the simulation, then type "cd src"  and hit Enter to update the simulation. Type "make". Hit Enter. Repeat steps 1-5 of "b) Running simulation with configuration file" for running the simulation.
 *
 * III. Running a simulation with a visualizer:
 ...
 *
 * To compile the program, change directory to repo-le000043/project/src. Then type "make".Then hit Enter.This builds both simulation with and without visualizer.
 * To run the program with visualizer, change directory to repo-le000043/project. Run ./build/bin/configuration_sim to run configuration simulation. Run ./build/bin/vis_sim <port_number>. A port number has to be greater than 8000 and not multiples of 10. Open browser and go to http://127.0.0.1:<port_number>/web_graphics/project.html 
 * To run the program without visualizer, change directory to repo-le000043/project. Run ./build/bin/configuration_sim <config_filename> <length of simulation> <time between busses for first route> <time between for second route>.
 * To modify, configuration file, refer to "a) Overview of Configuration file" in User's guide.
 * Importantly, all simulation has to be run from project directory instead of build/bin directory.
 * 
 * IV. TESTING
 * A. Regression Test
 *
 ...
 * B. Unit tests
 *
 * To run all unit tests, change directory to project/tests. Then run make. Lastly, run ../build/bin/unittest
 * 
 * V. OTHER NOTES:
 * For those who want to contribute to this project, such as adding a bus skipping stop feature, users can make changes in the Move method. If users want to improve the random generating passengers algorithm, users can modify random passenger generator class or create a new random passenger generator.
 * A. Creating new random passenger generator:
 *
 * 1. If the original random passenger generator header file is no longer needed, users need to change the file dependancy from random_passenger_generator.h to the file name of the new random passenger generator header file.
 * 2. If the source file of the random passenger generator is changed, then the original source file of random passenger generator has to be either renamed or deleted.
 * 3. Re-run all unit and regression tests to make sure the new random passsenger generator does not alter any main functionality of the simulation.
 * 
 * B. Creating new simulator:
 *
 * 1. Create new simulator header and simulator source file that have the same code as local_simulator.h and local_simulator.cc.
 * 2. In driver directory, create a new driver that utilizes the new local simulator that was just created in previous step.
 * 3. Then, make necessary modifications as new developers see fit.
 * 4. New simulator is expected to not change functionality of four main classes.
 */