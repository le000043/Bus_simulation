...
int RandomPassengerGenerator::GeneratePassengers() {
  ...
  stop_iter--;
  /*this override the last probability*/
  generation_probabilities_.pop_back();  // pop the last
  generation_probabilities_.push_back(0);  // push zero at the last
  /*done overriding the last probability*/
  int last_stop_index = (*stop_iter)->GetId();
  // TODO(Staff): check for accuracy
  // std::cout << "inside RANDOM passenger generator\n";
  for (prob_iter = generation_probabilities_.begin(),
    ...
    // std::cout << "current_generation_probability: " << current_generation_probability << std::endl;
    while (current_generation_probability > .0001) {
      ...
      //  std::cout << "True/false..." <<(generation_value < current_generation_probability)<< std::endl;
    ...
    stop_index++;
    // std::cout << "after loop, current_generation_probability: " << current_generation_probability << std::endl;
  }
