...
int main() {
  ...
  int rounds = 40; //Number of rounds of generation to simulate in test
  ...
  for (int i = 0; i < rounds; i++) {
    ...
    if (EB_distance == 0) {
      ...
      std::cout << std::endl << "EB Distance to next stop: " << EB_distance << std::endl << std::endl;
    }
    else {
      std::cout << std::endl << "WB Distance to next stop: " << WB_distance << std::endl << std::endl;
    }
